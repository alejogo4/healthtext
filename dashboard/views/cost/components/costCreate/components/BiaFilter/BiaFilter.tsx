...
const BiaFilter: FC<Props> = ({ variant, onNextStep }) => {
            ...
            updatedVariants[variantIndex].insumos = updatedVariants[
              ...
              insumo.id === insumoId
                ? {
                    ...
                    extension: extractExtensionFromBase64(base64String) ?? ''
                  }
                : insumo
            );
    ...
    if (listSuppliesAdded.length > 0) {
      ...
      const results = await Promise.all(
        clothPhoto.map(async item => {
          const response = await saveBiaPhoto(item);
          return response;
        })
      );
      setLoading(false);
    ...
    <div className='mt-8'>
      <div className='grid grid-cols-[1fr_2fr] gap-4 mt-5'>
        ...
        <div>
          <Card>
            <CardHeader>
              <CardTitle className='text-base'>Insumos encotrados</CardTitle>
            </CardHeader>
            <CardContent>
